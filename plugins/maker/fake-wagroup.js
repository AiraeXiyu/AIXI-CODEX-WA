...
export default {
   ...
   async run(m, {
      ...
      try {
         ...
         const [name = m.pushName, members = `${randomInteger(1, 1024)}`, desc = 'AIXI CODEX Community', author = m.pushName, date = '27/05/26 11:11'] = text.split('|')
         if (!text)
            return m.reply(`👉🏻 *Example*: ${isPrefix + command} AIXI CODEX | 1024 | gatau | AIXI CODEX | 27/05/26 11:11`)
         m.react('🕒')
         let profilePicture
         if (mimetype)
